...
from freightdesk.triage import stub_triage
...
def test_triage_parses_edi_port_delay():
    t = stub_triage(
        "STATUS: CNTR MSKU7401229 SHPMT TRK-40021-A HELD PORT OF SAVANNAH "
        "REASON WX SEVERE RAIN EST DELAY 14HRS NEXT UPDATE 0600", "edi")
    assert t.shipment_ref == "TRK-40021-A"
...
def test_triage_pharma_escalation_is_severity_5():
    ...
    assert t.shipment_ref == "TRK-40045-A"
    assert t.severity == 5
...
def test_duplicate_suppressed():
    desk = Desk()
    raw = "STATUS: SHPMT TRK-40099-Z HELD SAVANNAH DELAY 5HRS"
...
def test_pharma_full_pipeline():
    ...
    assert rec.draft is not None and "TRK-40045-A" in rec.draft.email_subject
